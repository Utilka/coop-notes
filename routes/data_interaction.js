...
router.get('/get_user', (req, res) => {
    //GET /data/get_user?id_user=1
    let id_user = parseInt(req.query.id_user, 10);  // true
    db.di.get_user(id_user).then (function(user) {res.json(user)}).catch(function (reason) {
        console.log(reason)
    });;
});
router.get('/get_canvas', (req, res) => {
    ...
    db.di.get_canvas(id_canvas).then (function(canvas) {res.json(canvas)}).catch(function (reason) {
        console.log(reason)
    });;
});
...
router.post('/upd_user', (req, res) => {
    //POST /data/upd_user?user_id=1 body{nick:andrey}
...
router.post('/upd_connection', (req, res) => {
    ...
    let data = req.body;
    db.di.upd_connection(upd_conection_id, data).then(res.status(200).send('Connection updated!')).catch(
        res.status(505).send(reson)
    );
});
//--------------------------------------------
